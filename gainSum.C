...
void findAndFitPeaks(int i)
{
  ...
  {
    double width = 1000.; // set by eye
    if (j == 0)
    {
      theNominalGain = nominalQsumGain;
      if (vchan[i] > 8 && vchan[i] < 12)
      { // trigger
        width = 100.0;
        theNominalGain = nominalQsumTrigGain;
      }
    }
    ...
    printf("try to fit to hist %s peak # %u bin %i xbin %f val %f from %f to %f \n", h->GetName(), j + 1, bin, xbin, peakVal, fitStart, fitEnd);
 ...
 * macro main entry
 * ******************* */
void gainSum(int theChan = 13) // default all
{
  sdate = currentDate();
  ...
  fout = new TFile(Form("gainSum-%s-%s.root", tag.Data(), sdate.c_str()), "recreate");
  ...
  for (unsigned ih = 0; ih < hlist.size(); ++ih)
    printf("hist %i name %s \n", ih, hlist[ih]->GetName());
  if (hlist.size() == 0)
    return;
  ...
  {
    if (!TString(hlist[i]->GetName()).Contains("Sum"))
      continue;
    ...
    else
    {
      ...
      sipmNumber.push_back(vchan[i]);
      sipmNumberError.push_back(0);
      printf("YYYYYYYYY chan %i max bin %f \n", i, maxValue);
    }
