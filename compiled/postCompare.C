                                  ...
                                  double const_init = 0.0)
{
    ...
    // Set reasonable ranges to help convergence
    fitFunc->SetParLimits(0, 1.E-10, 1.0E4); // amplitude1 must be positive
    fitFunc->SetParLimits(1, 1.0E01, 1.0E6); // tau1 must be positive
    fitFunc->SetParLimits(2, 1.E-10, 1.0E4); // amplitude2 must be positive
    fitFunc->SetParLimits(3, 1.0E01, 1.0E6); // tau2 must be positive
    fitFunc->SetParLimits(4, 0, 1.0E-5);     // constant must be non-negative
    ...
    */
    ...
    for (int fnum = 0; fnum < 2; ++fnum)
    {
        ...
        for (unsigned i = 0; i < 12; ++i)
        {
            ...
            TF1 *result = fitExpPlusConst(hnorm[fnum][i], 1600, 15000);
            amplitude[fnum].push_back(result->GetParameter(0));
            tau[fnum].push_back(result->GetParameter(1));
            tauErr[fnum].push_back(result->GetParError(1));
            constant[fnum].push_back(result->GetParameter(2));
        }
    }
    for (int fnum = 0; fnum < 2; ++fnum)
        for (unsigned i = 0; i < 12; ++i)
    ...
    for (int fnum = 0; fnum < 2; ++fnum)
        for (unsigned i = 0; i < 12; ++i)
        {
            ...
            can->SetLogy(true);
            can->Print(Form("ExpFitFile%dChannel%d.pdf", fnum, i));
        }

    for (int fnum = 0; fnum < 2; ++fnum)
        for (unsigned i = 0; i < 12; ++i)
        {
            printf("file %i Channel %d: noise = %.4E, Amplitude = %.4E, Tau = %.2f +/- %.2f, Constant = %.4E\n",
                   fnum, i, noiseSum[fnum][i], amplitude[fnum][i], tau[fnum][i], tauErr[fnum][i], constant[fnum][i]);
        }
