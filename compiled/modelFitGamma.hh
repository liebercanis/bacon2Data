...
enum
{
  NCHAN = 13, // 12 SIPMS + 1 PMT
  NLEVEL = 5
};
static double distanceLevel[NLEVEL];
...
static int level(int ichan)
{
  ...
  if (ichan == 9 || ichan == 10 || ichan == 11)
    ilevel = 0;
  else if (ichan == 6 || ichan == 7 || ichan == 8)
    ilevel = 1;
...
{
  std::cout << " modelFit::showEff " << std::endl;
  int ilevel = level(ichan);
...
modelFit::modelFit(int theFit, int ichan, double ppm)
{
  ...
  std::cout << "defined function " << fp->GetName() << "  " << fp->GetTitle() << std::endl;
}
