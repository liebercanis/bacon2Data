...
void spulse::getPulse()
{
  ...
  for (int i = 0; i < nsamples; ++i)
  {
    ...
    double v = 0;
    if (i >= istart && i < istart + iwidth)
      v = sign;
    if (i >= istart + iwidth)
      v = -1.0 * sign / sratio * Exp(-double(i - istart - iwidth) * xtau);
    yval.push_back(v);
  }
...
spulse::spulse()
{
  ...
  for (unsigned iw = 0; iw < xval.size(); ++iw)
  {
    if (iw < start)
      swave.push_back(0);
    else if (iw > start && iw < start + width)
      swave.push_back(1.0 * sign);
    else
      swave.push_back(0.);
  }
  ...
  double norm = 0;
  printf("swave %lu \n", swave.size());
  for (unsigned iw = 0; iw < swave.size(); ++iw)
    norm += swave[iw];
  norm = 1.0;
  ...
  // invert rWave... why?
  for (unsigned iw = 0; iw < rWave.size(); ++iw)
    rWave[iw] = -rWave[iw];
  // make graphs
